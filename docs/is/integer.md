...
[![version](https://img.shields.io/npm/v/@zerodep/is-integer?color=blue)](https://www.npmjs.com/package/@zerodep/is-integer)
...
A performant utility to determine if a value is a finite integer and assign a `number` Typescript type to the value.
...
isInteger(gen1); // false
isInteger(gen2); // false
...
// Promise
isInteger(new Promise(() => 1)); // false
isInteger(Promise.resolve()); // false

// Regular Expression
isInteger(/[regx]+/gi); // false
