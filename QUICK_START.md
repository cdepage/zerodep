...
- **Testing:** is handled by Vitest. Test coverage of at least 85% is required for all code; efforts should be made to ensure edge cases are well tested, as well as the typical execution paths. Tests may be run by calling: `npm run test:all`; test coverage may be determined by calling `npm run coverage:all`.
