...
import { ZeroDepErrorIs } from '@zerodep/errors';
...
export const isEqual = (val1: any, val2: any): boolean => {
  const compare = (a: any, b: any): boolean => {
    ...
    switch (typeA) {
      ...
      case 'Error':
        // time complexity: constant - O(1)
        return a.name === b.name && a.message === b.message && typeof a === typeof b;
      ...
      default:
        const error = new ZeroDepErrorIs(`Cannot compare ${typeA} values`);
        error.value = a;
        throw error;
    }
  };
