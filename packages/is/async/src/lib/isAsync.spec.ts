...
describe('isAsync', () => {
  const exampleCases = [
    ...
    ['function - async', testFunctionAsync, true],
    ...
    ['promise', testPromise1, true],
    ['promise - all', testPromiseAll, true],
    ['promise - allSettled', testPromiseAllSettled, true],
    ['promise - race', testPromiseRace, true],
    ['promise - resolved', testPromiseResolved1, true],
  ...
  test.each(exampleCases)('should assess %s', async (title, value, result) => {
    expect(isAsync(value)).toEqual(result);
  });
});
